...
def run_rtklib_rel(rover_file: str, base_file: str, nav_file: str, out_file: str):
    """Запускает rnx2rtkp в режиме относительного позиционирования."""
    cmd = [
        "/Users/sergeidolin/RTKLIB/app/consapp/rnx2rtkp/gcc/rnx2rtkp",
        "-p", "3",  # Режим кинематики с поправками
        "-o", out_file,
        rover_file,  # Сначала ровер
        base_file,   # Потом база
        nav_file     # Эфемериды
    ]
...
def handle_client(conn):
    ...
    base_path = rover_path = eph_path = result_path = None
    
    try:
        # --- Прием базового файла ---
        print(f"[Сервер] Прием базового файла...")
        name_len = struct.unpack('>I', recv_exactly(conn, 4))[0]
        base_filename = recv_exactly(conn, name_len).decode('utf-8')
        base_file_size = struct.unpack('>Q', recv_exactly(conn, 8))[0]
        
        # Сохраняем базовый файл во временный файл
        with tempfile.NamedTemporaryFile(delete=False, suffix='.obs', prefix='base_') as f:
            base_path = f.name
            total = 0
            while total < base_file_size:
                chunk = conn.recv(min(65536, base_file_size - total))
                if not chunk:
                    raise RuntimeError("Клиент разорвал соединение")
                f.write(chunk)
                total += len(chunk)
        
        print(f"[Сервер] Базовый файл сохранен: {base_path}")
        
        # --- Прием роверного файла ---
        print(f"[Сервер] Прием роверного файла...")
        name_len = struct.unpack('>I', recv_exactly(conn, 4))[0]
        rover_filename = recv_exactly(conn, name_len).decode('utf-8')
        rover_file_size = struct.unpack('>Q', recv_exactly(conn, 8))[0]

        # Сохраняем роверный файл во временный файл
        with tempfile.NamedTemporaryFile(delete=False, suffix='.obs', prefix='rover_') as f:
            rover_path = f.name
            total = 0
            while total < rover_file_size:
                chunk = conn.recv(min(65536, rover_file_size - total))
                if not chunk:
                    raise RuntimeError("Клиент разорвал соединение")
                f.write(chunk)
                total += len(chunk)
        
        print(f"[Сервер] Роверный файл сохранен: {rover_path}")
        
        # --- Обработка файлов ---
        # Извлекаем дату из любого файла (они должны быть одного дня)
        obs_date = extract_date_from_rinex(base_path)
        print(f"[Сервер] Дата наблюдений: {obs_date}")
        
        # Скачиваем эфемериды
        temp_dir = os.path.dirname(base_path)
        eph_path = download_brdc_from_bkg(obs_date, out_dir=temp_dir)
        print(f"[Сервер] Эфемериды скачаны: {eph_path}")
        
        # Запускаем RTKLIB в режиме относительного позиционирования
        result_path = rover_path.replace('.obs', '.pos')
        print(f"[Сервер] Запуск RTKLIB...")
        ...
        print(f"[Сервер] Отправка результата клиенту...")
        conn.sendall(b"OK::")
        conn.sendall(struct.pack('>Q', len(result_data)))
        conn.sendall(result_data)
        print(f"[Сервер] Обработка завершена успешно")
        ...
        print(f"[Сервер] Ошибка: {error_msg}")
        try:
        ...
        for p in [base_path, rover_path, eph_path, result_path]:
            if p and os.path.exists(p):
                try:
                    os.remove(p)
                    print(f"[Сервер] Удален временный файл: {p}")
                except Exception as e:
                    print(f"[Сервер] Не удалось удалить {p}: {e}")


def main():
    server = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    server.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
    server.bind(('0.0.0.0', 9999))  # Слушаем на всех интерфейсах
    server.listen(5)
